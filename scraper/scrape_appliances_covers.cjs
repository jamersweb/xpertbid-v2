...
const OUTPUT_JSON_PATH = path.join(SCRAPER_DIR, 'appliances_covers_products.json');
...
async function scrapeAppliancesCovers() {
  console.log('Starting scraper for Appliances Covers on ctexs.pk...');
  let allProducts = [];
  
  const collectionHandles = ['appliances-covers', 'appliance-covers', 'appliances-cover'];
  ...
  console.log(`\nProcessing ${allProducts.length} total Appliances Cover products...`);
  ...
  for (let i = 0; i < allProducts.length; i++) {
    ...
    imagesUrl.forEach((imgUrl, imgIndex) => {
      ...
      const filename = `appliance_cover_${prod.id}_img_${imgIndex + 1}.${ext}`;
  ...
  console.log(`\n✅ Successfully scraped ${structuredProducts.length} Appliances Cover products!`);
  console.log(`📁 Saved product JSON to: ${OUTPUT_JSON_PATH}`);
  console.log(`🖼️ Saved downloaded images to: ${IMAGES_DIR}`);
}

scrapeAppliancesCovers().catch(err => {
