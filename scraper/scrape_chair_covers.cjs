...
const OUTPUT_JSON_PATH = path.join(SCRAPER_DIR, 'chair_covers_products.json');
...
async function scrapeChairCovers() {
  console.log('Starting scraper for Chair Covers on ctexs.pk...');
  let allProducts = [];
  let page = 1;
  let hasMore = true;

  while (hasMore) {
    const collectionUrl = `https://ctexs.pk/collections/chair-covers/products.json?limit=250&page=${page}`;
  ...
  console.log(`\nProcessing ${allProducts.length} total Chair Cover products...`);
  ...
  for (let i = 0; i < allProducts.length; i++) {
    ...
    imagesUrl.forEach((imgUrl, imgIndex) => {
      ...
      const filename = `chair_cover_${prod.id}_img_${imgIndex + 1}.${ext}`;
