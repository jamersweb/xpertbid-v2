...
const productUrls = [
  ...
  "https://www.daraz.pk/products/messi-2027-i1968171071.html",
  "https://www.daraz.pk/products/202526-9-i1968167110.html",
  "https://www.daraz.pk/products/202526-i1968153489.html",
  "https://www.daraz.pk/products/ac-200607-80-i1968145815.html",
  "https://www.daraz.pk/products/2026-7-i1967989311.html"
];
...
async function parseProductData(html, url, imagesDir) {
  ...
  // Title
  const title = productObj.title || trackingObj.pdt_name || (schemaData ? schemaData.name : 'Football Jersey');
  ...
  const category = categoryArr.join(' > ') || (schemaData ? schemaData.category : 'Sports & Outdoors > Shoes & Clothing > Men > Clothing > Jerseys');
  ...
  for (let idx = 0; idx < imagesUrl.length; idx++) {
    ...
    const filename = `football_shirt_${id}_img_${idx + 1}${cleanExt}`;
  ...
  if (Array.isArray(skuBase.skus) && skuBase.skus.length > 0) {
    skuBase.skus.forEach((skuItem, idx) => {
      ...
      const variantTitle = optionValues.length > 0 ? optionValues.join(' / ') : `Standard / Size ${idx + 1}`;
...
async function scrapeAll() {
  ...
  console.log(`Starting to scrape ${productUrls.length} Daraz products provided by user...`);
  const results = [];
  ...
  const outputPath = path.join(__dirname, 'daraz_football_shirts_products.json');
