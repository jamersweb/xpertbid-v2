...
const OUTPUT_JSON_PATH = path.join(SCRAPER_DIR, 'fitted_sheets_products.json');
...
async function runConcurrently(tasks, limit = 15) {
  const results = [];
  const executing = [];
...
async function scrapeFittedSheets() {
  console.log('Starting scraper for Fitted Sheets on ctexs.pk...');
  let allProducts = [];
  let page = 1;
  let hasMore = true;

  while (hasMore) {
    const collectionUrl = `https://ctexs.pk/collections/fitted-sheets/products.json?limit=250&page=${page}`;
  ...
  // Prepare all image download tasks first
  const downloadTasks = [];
  ...
  for (let i = 0; i < allProducts.length; i++) {
    ...
    imagesUrl.forEach((imgUrl, imgIndex) => {
      ...
      const filename = `fitted_sheet_${prod.id}_img_${imgIndex + 1}.${ext}`;
