...
const OUTPUT_JSON_PATH = path.join(SCRAPER_DIR, 'curtains_products.json');
...
async function scrapeCurtains() {
  console.log('Starting scraper for Curtains on ctexs.pk...');
  let allProducts = [];
  
  const collectionHandles = ['curtains', 'curtain'];
  ...
  console.log(`\nProcessing ${allProducts.length} total Curtain products...`);
  ...
  for (let i = 0; i < allProducts.length; i++) {
    ...
    imagesUrl.forEach((imgUrl, imgIndex) => {
      ...
      const filename = `curtain_${prod.id}_img_${imgIndex + 1}.${ext}`;
