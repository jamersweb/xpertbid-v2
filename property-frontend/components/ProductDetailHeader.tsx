...
export function ProductDetailHeader({ views = 0, slug, shareUrl }: Props) {
  const handleCopyLink = () => {
    const url =
      shareUrl ||
      (typeof window !== "undefined" ? `${window.location.origin}/properties/${slug}` : "");
    if (!url) return;
    navigator.clipboard
      .writeText(url)
  ...
  return (
    <section className="prodcut-detail-links product-detail-header-section">
