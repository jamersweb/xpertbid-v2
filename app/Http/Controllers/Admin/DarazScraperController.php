...
use App\Models\User;
use App\Services\DarazScraperService;
...
{
    public function __construct(protected DarazScraperService $scraper)
    {
    }
    ...
    public function index(Request $request)
    {
        return Inertia::render('Admin/Tools/OlxScraper', $this->pageProps([
            'tool_name' => 'Daraz Scraper',
            'tool_short' => 'Daraz',
            'url' => $request->string('url')->toString(),
            'status' => $request->session()->get('success') ?? $request->session()->get('status'),
            'error' => $request->session()->get('error'),
            'preview_route' => 'admin.daraz-scraper.preview',
            'save_route' => 'admin.daraz-scraper.save',
            'image_route' => 'admin.daraz-scraper.image',
            'url_placeholder' => 'https://www.daraz.pk/...'
            , 'default_listing_type' => 'normal'
        ]));
    }
    ...
    public function preview(Request $request)
    {
        ...
        try {
            ...
            $error = empty($preview['title']) ? 'Daraz listing title was not detected from the page HTML.' : null;
        } catch (\Throwable $e) {
            Log::warning('Daraz preview scrape failed', [
                'url' => $validated['url'],
    ...
    public function save(Request $request)
    {
        ...
        try {
            ...
            Log::error('Daraz save scrape failed', [
                'url' => $validated['url'],
                'message' => $e->getMessage(),
            ]);
        ...
        $listingType = $validated['listing_type'] ?? 'normal';
        ...
        if ($title === '' || $title === 'No title') {
            return back()->withInput()->with('error', 'Daraz title could not be detected. Please enter it manually.');
        }
        ...
        return redirect()
            ->route('admin.daraz-scraper.index', ['url' => $validated['url']])
            ->with('success', 'Daraz listing saved successfully. Listing ID: ' . $listing->id);
    }
