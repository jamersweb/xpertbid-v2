...
class DarazShoesListingSeeder extends Seeder
{
    /**
    ...
    {
        $jsonPath = base_path('scraper/daraz_shoes_products.json');
        ...
        $this->command?->info("Starting seeding for {$totalCount} Daraz Footwear products with discount calculation...");

        foreach ($productsToSeed as $index => $prod) {
            $productNum = $index + 1;
            $title = $prod['title'] ?? "Daraz Footwear Product {$productNum}";
            ...
            foreach ($imageSources as $imgIdx => $imgSource) {
                ...
                $filename = "shoe_{$prod['id']}_img_" . ($imgIdx + 1) . '.' . pathinfo(parse_url($imgSource, PHP_URL_PATH) ?: 'jpg', PATHINFO_EXTENSION);
                // Sanitize extension
            ...
            if ($descriptionHtml && preg_match_all('/<img[^>]+src=["\']([^"\']+)["\']/i', $descriptionHtml, $matches)) {
                ...
                foreach ($htmlImageUrls as $htmlImgIdx => $htmlImgUrl) {
                    ...
                    $descImgName = "shoe_{$prod['id']}_desc_" . ($htmlImgIdx + 1) . ".{$ext}";
        ...
        $this->command?->info("✅ Daraz Shoes seeder executed successfully for {$totalCount} products with discount calculation.");
    }
}
