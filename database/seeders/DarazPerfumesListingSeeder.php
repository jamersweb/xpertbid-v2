...
class DarazPerfumesListingSeeder extends Seeder
{
    /**
    ...
    {
        $jsonPath = base_path('scraper/daraz_perfumes_products.json');
        ...
        $this->command?->info("Starting seeding for {$totalCount} Daraz Perfume products with discount calculation...");

        foreach ($productsToSeed as $index => $prod) {
            $productNum = $index + 1;
            $title = $prod['title'] ?? "Daraz Perfume Product {$productNum}";
            ...
            foreach ($imageSources as $imgIdx => $imgSource) {
                ...
                $filename = "perfume_{$prod['id']}_img_" . ($imgIdx + 1) . '.' . pathinfo(parse_url($imgSource, PHP_URL_PATH) ?: 'jpg', PATHINFO_EXTENSION);
                // Sanitize extension
            ...
            if ($descriptionHtml && preg_match_all('/<img[^>]+src=["\']([^"\']+)["\']/i', $descriptionHtml, $matches)) {
                ...
                foreach ($htmlImageUrls as $htmlImgIdx => $htmlImgUrl) {
                    ...
                    $descImgName = "perfume_{$prod['id']}_desc_" . ($htmlImgIdx + 1) . ".{$ext}";
        ...
        $this->command?->info("✅ Daraz Perfumes seeder executed successfully for {$totalCount} products with discount calculation.");
    }
}
