...
class CurtainsListingSeeder extends Seeder
{
    /**
    ...
    {
        $jsonPath = base_path('scraper/curtains_products.json');
        ...
        $this->command?->info("Starting fast seeding for {$totalCount} Curtain products with discount data...");

        foreach ($productsToSeed as $index => $prod) {
            $productNum = $index + 1;
            $title = $prod['title'] ?? "Curtain Product {$productNum}";
            ...
            foreach ($localImages as $imgIdx => $imgSource) {
                ...
                $filename = "curtain_{$prod['id']}_img_" . ($imgIdx + 1) . '.' . pathinfo(parse_url($imgSource, PHP_URL_PATH) ?: 'jpg', PATHINFO_EXTENSION);
                // Sanitize extension
        ...
        $this->command?->info("✅ Curtains seeder executed successfully for {$totalCount} products with discount calculation.");
    }
}
