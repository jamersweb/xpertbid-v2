...
class DarazFootballShirtsListingSeeder extends Seeder
{
    /**
    ...
    {
        $jsonPath = base_path('scraper/daraz_football_shirts_products.json');
        ...
        $this->command?->info("Starting seeding for {$totalCount} Daraz Football Shirts with discount calculation...");

        foreach ($productsToSeed as $index => $prod) {
            $productNum = $index + 1;
            $title = $prod['title'] ?? "Daraz Football Shirt {$productNum}";
            $this->command?->info("[{$productNum}/{$totalCount}] Seeding: {$title}");
            ...
            foreach ($imageSources as $imgIdx => $imgSource) {
                ...
                $filename = "football_shirt_{$prod['id']}_img_" . ($imgIdx + 1) . '.' . pathinfo(parse_url($imgSource, PHP_URL_PATH) ?: 'jpg', PATHINFO_EXTENSION);
                // Sanitize extension
            ...
            if ($descriptionHtml && preg_match_all('/<img[^>]+src=["\']([^"\']+)["\']/i', $descriptionHtml, $matches)) {
                ...
                foreach ($htmlImageUrls as $htmlImgIdx => $htmlImgUrl) {
                    ...
                    $descImgName = "football_shirt_{$prod['id']}_desc_" . ($htmlImgIdx + 1) . ".{$ext}";
        ...
        $this->command?->info("✅ Daraz Football Shirts seeder executed successfully for {$totalCount} products with discount calculation.");
    }
}
