...
class AppliancesCoversListingSeeder extends Seeder
{
    /**
    ...
    {
        $jsonPath = base_path('scraper/appliances_covers_products.json');
        ...
        $this->command?->info("Starting fast seeding for {$totalCount} Appliances Cover products with discount data...");

        foreach ($productsToSeed as $index => $prod) {
            $productNum = $index + 1;
            $title = $prod['title'] ?? "Appliance Cover Product {$productNum}";
            ...
            foreach ($localImages as $imgIdx => $imgSource) {
                ...
                $filename = "appliance_cover_{$prod['id']}_img_" . ($imgIdx + 1) . '.' . pathinfo(parse_url($imgSource, PHP_URL_PATH) ?: 'jpg', PATHINFO_EXTENSION);
                // Sanitize extension
        ...
        $this->command?->info("✅ Appliances Covers seeder executed successfully for {$totalCount} products with discount calculation.");
    }
}
