...
class FittedSheetsListingSeeder extends Seeder
{
    /**
    ...
    {
        $jsonPath = base_path('scraper/fitted_sheets_products.json');
        ...
        $this->command?->info("Starting seeding for {$totalCount} Fitted Sheet products with discount data...");

        foreach ($productsToSeed as $index => $prod) {
            $productNum = $index + 1;
            $title = $prod['title'] ?? "Fitted Sheet Product {$productNum}";
            ...
            foreach ($imageSources as $imgIdx => $imgSource) {
                ...
                $filename = "fitted_sheet_{$prod['id']}_img_" . ($imgIdx + 1) . '.' . pathinfo(parse_url($imgSource, PHP_URL_PATH) ?: 'jpg', PATHINFO_EXTENSION);
                // Sanitize extension
            ...
            if ($descriptionHtml && preg_match_all('/<img[^>]+src=["\']([^"\']+)["\']/i', $descriptionHtml, $matches)) {
                ...
                foreach ($htmlImageUrls as $htmlImgIdx => $htmlImgUrl) {
                    ...
                    $descImgName = "fitted_sheet_{$prod['id']}_desc_" . ($htmlImgIdx + 1) . ".{$ext}";
        ...
        $this->command?->info("✅ Fitted Sheets seeder executed successfully for {$totalCount} products with discount calculation.");
    }
}
