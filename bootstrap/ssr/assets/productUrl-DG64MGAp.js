    ...
    if (!user) {
      try {
        ...
        if (productDetails) {
          ...
          if (variationId !== null && variationId !== void 0 && listing.variations) {
            const foundVar = listing.variations.find((v) => String(v.id) == String(variationId)) || listing.listing_data?.variations?.[variationId];
            if (foundVar) {
