...
function getBaseListingPrice(item) {
  ...
  const variationPrices = variations.map((variation) => Number(variation?.price)).filter((value) => Number.isFinite(value) && value > 0);
  if (variationPrices.length > 0) {
    return Math.min(...variationPrices);
  }
  return 0;
}
function getCardDisplayPrice(item) {
...
function getListingVariations(item) {
  const raw = Array.isArray(item?.variations) && item.variations.length > 0 ? item.variations : Array.isArray(item?.listing_data?.variations) ? item.listing_data.variations : [];
...
function getVariationPriceMeta(variation, item) {
  ...
  return {
    ...
    badgeText: hasDiscount ? discountType === "percent" ? `${Math.round(discountValue)}% OFF` : "SALE" : ""
  };
}
export {
  isDirectBuyListing as a,
  getBaseListingPrice as b,
  getListingVariations as c,
  getVariationPriceMeta as d,
  getCardDisplayPrice as e,
