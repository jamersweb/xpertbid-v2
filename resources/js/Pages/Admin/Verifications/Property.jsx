...
export default function Property({ verifications }) {
       ...
       return (
              <AdminLayout title="Property Verifications">
                     <Head title="Property Verifications" />
                                                 ...
                                                 {verifications.data.map((verification) => (
                                                        <tr key={verification.id} className="hover:bg-gray-50/50 transition-colors">
                                                               <td className="px-6 py-4">
                                                                      <p className="text-sm font-bold text-gray-800">{verification.property_type}</p>
                                                                      <p className="text-[11px] text-gray-500 truncate max-w-xs">{verification.property_address}</p>
                                                                      <p className="text-[10px] text-amber-600 font-bold">Deed: {verification.title_deed_number}</p>
                                                               </td>
                                                               <td className="px-6 py-4">
                                                               ...
                                                               <td className="px-6 py-4">
                                                                      <div className="flex gap-1 flex-wrap max-w-[150px]">
                                                                             {(verification.property_documents || []).map((doc, idx) => (
                     ...
                     <Modal show={isModalOpen} onClose={() => setIsModalOpen(false)} maxWidth="md">
                            <div className="p-6">
                                   <h2 className="text-lg font-bold text-gray-800 mb-4">Decline Property Verification</h2>
                                   <div className="mb-6">
